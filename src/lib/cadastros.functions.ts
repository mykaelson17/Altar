...
export const listCargos = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    return await q<{ id: string; nome: string }>(`SELECT id, nome FROM cargos ORDER BY nome ASC`);
  });
...
export const deleteCargo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await q(`DELETE FROM cargos WHERE id = ?`, [data.id]);
    return { success: true };
  });
...
export const listDepartamentos = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(async () => {
    return await q<{ id: string; nome: string }>(`SELECT id, nome FROM departamentos ORDER BY nome ASC`);
  });
...
export const deleteDepartamento = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ id: z.string().min(1) }).parse(d))
  .handler(async ({ data }) => {
    await q(`DELETE FROM departamentos WHERE id = ?`, [data.id]);
    return { success: true };
  });
...
export const listTiposCulto = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(async () => {
    return await q<{ id: string; nome: string }>(`SELECT id, nome FROM tipos_culto ORDER BY nome ASC`);
  });
...
export const deleteTipoCulto = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ id: z.string().min(1) }).parse(d))
  .handler(async ({ data }) => {
    await q(`DELETE FROM tipos_culto WHERE id = ?`, [data.id]);
    return { success: true };
  });
...
export const listTiposEvento = createServerFn({ method: "GET" })
  .middleware([requireAuth])
  .handler(async () => {
    return await q<{ id: string; nome: string }>("SELECT id, nome FROM tipos_evento ORDER BY nome");
  });
...
export const createTipoEvento = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      await q("INSERT INTO tipos_evento (nome) VALUES ($1)", [data.nome.trim()]);
    } catch (e: any) {
...
export const deleteTipoEvento = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await q("DELETE FROM tipos_evento WHERE id = $1", [data.id]);
    return { success: true };
  });
