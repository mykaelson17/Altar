...
for (const file of files) {
  if (file.endsWith('.ts') && file !== 'sqlite.server.ts' && file !== 'db.server.ts') {
    ...
    // Replace q( and q1( with await q( and await q1(
    let nc = c.replace(/(?<!await\s+)(?<!function\s+)(?<!export\s+)(?<!\w)(q1?)\s*\(/g, 'await $1(');
    
    // Replace q< and q1< with await q< and await q1<
    nc = nc.replace(/(?<!await\s+)(?<!function\s+)(?<!export\s+)(?<!\w)(q1?)\s*</g, 'await $1<');
